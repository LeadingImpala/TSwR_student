...
class ManiuplatorModel:
    def __init__(self, Tp):
        ...
        self.d1 = self.l1/2
        self.d2 = self.l2/2
        #self.alpha = self.m1*np.square(self.l1/2) + self.I_1 + self.m2*(np.square(self.l1)+np.square(self.l2/2)) + self.I_2
        #self.beta = self.m2 * self.l1 * self.l2/2
        #self.gamma = self.m2 * np.square(self.l2/2) + self.I_2
        self.alpha = self.m1*self.d1**2 + self.m2*(self.l1**2+self.d2**2 + self.l2**2)+self.m3*self.l1**2+self.I_1+self.I_2+self.I_3
        self.beta = 2*self.l1*self.d2*self.m2+2*self.l1*self.l2*self.m3
        self.gamma = self.m2*self.d2**2 + self.I_2+self.m2*self.I_2**2+self.I_3
